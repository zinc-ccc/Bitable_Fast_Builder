...
### 4. 遇权限拦截立刻停止内耗（防 Token 浪费）
**[对 Agent 的强指令要求]**：在执行自动化代码时，如果遇到 API 报错提示**权限不足、Token 校验失败、或环境拦截**等明确涉及客观权限问题的报错，**立刻停止盲目的代码重试与逻辑修改**。你必须第一时间分析并给出解决思路，直接要求用户去飞书后台或 IT 部门去申请/核实对应的权限，绝不能在原地反复修改代码进行无效调试，以免造成严重的 Token 资源消耗与时间浪费。
