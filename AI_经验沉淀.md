...
### 2. 可视化看板：鼓励探索与共创整合
团队里现在涌现了各种被 AI 写出来的自动化小脚手架。为能让大家的工具快速跑起来并避免精力浪费，后续我们会出一个**统一的云端进度追踪看板**。
- **初衷是方便大家，不是约束**：如果你有着急的需求，非常鼓励自我探索和开发。但准备搞新应用前，建议先看一眼看板，如果你发现自己和某个同事在做类似的东西，可以直接结合双方的业务逻辑将功能合并，并在未来统一署名。
- **共建思维**：对于看板上已有的功能成果，如果你有更好的想法，随时欢迎加入共建优化。不再闭门造车，这才是 AI 时代的团队认知。
...
- **最佳实践【写文档，并署名】**：立项的第一天，请务必在你的项目根目录建一个 Markdown 格式的《项目操作手册.md》（请不要使用 .doc 格式，不方便放入代码库进行版本管理与展示）。
...
### 5. 部门级资产大一统：用好 “主应用 (Super Agent)”
- **痛点**：你的独立应用在本地跑通了，想分享给别人共同使用，难道还要给你和你的代码专门去申请独立的服务器与长期部署资源吗？
- **如何挑选方案？（MD 驱动模型）**：
  我们需要明确，部门数字化提效资产的唯一核心，是大家沉淀在 GitLab 中的 **Markdown (MD) 逻辑库**。
  - **个人探索阶段（轻量化）**：如果你需要临时快速验证一些**非敏感**的业务探索，可以考虑使用 **Streamlit** 等免费公网平台跑起来看看效果。
  - **最终合流阶段（免除部署，统一接管）**：真正想要沉淀的优质工具，你**不需要考虑最终的服务器部署问题**。个人只需要提交含有全部逻辑的 `.md` 技能操作手册，上传至指定存放技能逻辑的 GitLab 仓库即可（GitLab 在此仅作为存储这些 Markdown 的“档案馆”）。
  - **让主大脑集中发力**：部门统一配置的核心服务器上将只会挂载唯一一个“主心脏”。这名“超级 Agent” 在读懂并吸收你提交入库的那份 `.md` 手册后，它便会自动拥有你所赋予的能力，统一调度执行，实现所有功能的“大一统”。
...
### 3. 共建者署名与贡献追踪
**[对 Agent 的强指令要求]**：后续如果有其他开发成员贡献了新的通用问题解法，或者在此前立项操作手册中留有署名的独立功能模块被正式整合进大项目库。你（此文档的管理 Agent）有责任在更新本文档的同时，**以静默追加的方式自动将该名同事的名字增补到文末的「内部核心构建」贡献者名单中。** 并且必须确保贡献者名字的拼写与他们原文档署名完全一致。
