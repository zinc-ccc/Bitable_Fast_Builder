...
- **关于安全与权限的【最小化原则】**：
  ...
  **[温馨提示]**：为了大家的安全，建议各位在使用时尽量别将机器人拉入包含机密、薪资或敏感商机的群聊中哦~ 如果自动化流程实在需要，拉入前也请一定要了解清楚机器人能读到什么信息。一方面这能杜绝咱们的数据外泄风险，另一方面如果权限申请得刚刚好，集团那边的审核阻力也会小很多。

### 4. 规范先行：让你的小项目成为部门基建的“乐高积木” (协同与操作手册原则)
- **痛点**：大家都在用 AI 摸索各种提效小工具，如果各自为战，不仅功能容易撞车，后续若要把好用的小工具整合进部门统一系统时，将会面临极大的重构痛苦。
- **最佳实践【文档当家，AI 维护】**：为了后续能轻松合并整合，在大家折腾小项目的第一天起，就**必须建立一份《项目操作手册》或说明文档**。
  - **文档要点**：说清楚你的功能设计逻辑、想要/已实现的效果、以及采用了什么关键技术路径。
  - **[对 Agent 的强指令要求]**：当用户在当前目录下立项开发新工具时，你（Agent）必须在根目录下主动初始化这份操作说明。在后续陪同用户开发跑项目的全过程中，你（Agent）有责任结合每一次的调试，**自动、实时地把最新逻辑更新进这份说明文档中，绝不能让用户自己去费神手写。**
  - **最终红利**：当你的工具跑通后，只需要提交这份由 AI 打理好的文档。后续的开发者只要把这份说明喂给 AI，就能轻松地将你的成果像“搭乐高”一样整合进部门大平台。

### 5. 帮你把应用部署到云端，让团队一起用
- **痛点**：代码终于在本地跑通了，但别人用不了、关机就罢工？
- **如何挑选部署方案？**：
  - **公开通用小网页（免费且极速）**：如果你的工具没有涉及任何内部敏感数据，只是想做个方便的网站给大家用，可以考虑用 **Streamlit** 这类免费平台托管。你只需要注册一个 GitHub 账号剩下的交给 AI。🗣️ *发问参考：“我想把这段代码做成一个可通过网页访问的 Streamlit 应用并免费部署到公网，请一步步教我怎么操作。”*
  - **团队内部合规应用（推荐）**：如果工具需要让不同工区（不同 IP 属地）的 HR 同事一起高频使用，且涉及敏感表单，未来请尽量部署到咱们**公司的 Gitlab 或内部云服务器上**，这样既安全又稳定，后续会专门出这部分的“傻瓜教程”带大家玩转内部基建。
