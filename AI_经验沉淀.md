...
- **原理解释与操作规范（小白必读）**：
  ...
  - **防撞车解法：加专属指令缩写**：为了方便大家免去重复审批的流程、避免内部机器人管理散乱，推荐大家直接合用团队通用机器人。合用时，只需在给 AI 设置触发规则时加上**专属指令缩写**（例如设定触发词为：`Zinc发周报`、`Tom查考勤`）。让各个程序的 AI 忽略掉不带自己标识的消息，机器人就不会互相干扰了。当然，如果个人有特殊的独立业务需求，您依然可以单独去申请自己的专属机器人，不局限于一定要合用。
- **关于安全与权限的【最小化原则】**：
