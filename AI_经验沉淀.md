...
⚠️ **【特别预警：Token 消耗预警与权限控制】**
- **极度烧钱提醒**：作为数字员工，OpenClaw 会为了执行任务在后台不断对大模型发起推理请求（思考下一步该干嘛）。**这会飞速消耗你的 API 额度（真实花费）！** 强烈建议一开始使用时，在 API 平台**一定要设置消费封顶预警**，以免一夜醒来额度耗尽。
- **权限红线**：它有极高的系统权限。切勿一开始就让它处理财务数据，必须从小任务（如建文件夹）开始磨合建立信任感。
