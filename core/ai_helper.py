...
class AIHelper:
    ...
    def summarize_global_agenda(self, all_agendas_text: str) -> str:
        system_prompt = (
            ...
            "用户提供的是本组各BP个人的AI议程总结，请严格按照“总-分形式”输出汇总提炼：\n"
            "【总】：用一两句话总结本组目前的全局共性问题、核心风险或整体趋势，提醒负责人关注。\n"
            "【分】：点名到个人，具体指出谁的哪个模块存在卡点/风险（例如：@某某：三区出现集中离职风险等）。\n"
            "请保持专业、精炼，无需客套，直接输出【总】和【分】的内容排版。"
        )
        try:
            resp = self.client.chat.completions.create(
                model=self.model,
